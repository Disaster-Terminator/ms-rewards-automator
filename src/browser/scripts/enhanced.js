/**
 * Enhanced Anti-Focus Script
 *
 * 用途：防止浏览器窗口自动获取焦点，适用于无头模式下的自动化任务
 *
 * ⚠️ 警告：
 * - 此脚本会禁用所有焦点相关方法（focus, blur, scrollIntoView）
 * - 可能影响输入框的聚焦和键盘输入
 * - 仅在 prevent_focus='enhanced' 配置下启用
 * - 不推荐在需要用户交互的场景下使用
 *
 * 建议使用场景：
 * - 纯无头模式自动化
