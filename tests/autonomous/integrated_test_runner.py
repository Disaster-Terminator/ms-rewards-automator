...
from .page_inspector import DetectedIssue, IssueSeverity, IssueType, PageInspector
...
class IntegratedTestRunner:
    ...
    def __init__(self, config_path: str = "config.yaml", user_mode: bool = False):
        ...
        self.points_tracking = {"initial": None, "final": None, "gained": 0, "history": []}
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.config = ConfigManager(
                self.config_path, dev_mode=not self.user_mode, user_mode=self.user_mode
            )
    ...
    async def _wrapped_execute(self, original_run):
        ...
        try:
            ...
            if self.app and self.app.page:
                await self.screenshot_manager.capture_on_error(self.app.page, e, "test_execution")
    ...
    async def _step_check_points(self):
        ...
        try:
            ...
            if hasattr(self.app, "state_monitor") and self.app.state_monitor:
                self.points_tracking["initial"] = getattr(
                    self.app.state_monitor, "initial_points", None
                )
        ...
        try:
            ...
            if current_points is not None:
                ...
                if self.points_tracking["initial"]:
                    ...
                    logger.info(
                        f"  📊 搜索后积分: {current_points} (变化: {'+' if gained >= 0 else ''}{gained})"
                    )
    ...
    async def _step_execute_tasks(self):
        ...
        try:
            ...
            if current_points is not None:
                ...
                if self.points_tracking["initial"]:
                    self.points_tracking["gained"] = (
                        current_points - self.points_tracking["initial"]
                    )
                    logger.info(
                        f"  📊 最终积分: {current_points} (总变化: {'+' if self.points_tracking['gained'] >= 0 else ''}{self.points_tracking['gained']})"
                    )
    ...
    async def _get_current_points(self) -> int | None:
        """获取当前积分"""
        try:
    ...
    async def _capture_issue_screenshots(
        self, issues: list[DetectedIssue], base_screenshot: str | None = None
    ):
    ...
    async def _record_step(
        self, name: str, success: bool, start_time: float, error: str | None = None
    ):
        """记录测试步骤"""
    ...
    def generate_reports(self) -> dict[str, str]:
        ...
        text_report_path = (
            Path(self.reporter.output_dir) / f"test_report_{self.reporter.session_id}.txt"
        )
        ...
        return {"json": json_report, "html": html_report, "text": str(text_report_path)}
