...
class TestBingThemeManager:
        ...
        with patch.object(theme_manager, "_detect_theme_by_css_classes", return_value="dark"):
            with patch.object(theme_manager, "_detect_theme_by_computed_styles", return_value=None):
                with patch.object(theme_manager, "_detect_theme_by_cookies", return_value=None):
                    with patch.object(
                        theme_manager, "_detect_theme_by_url_params", return_value=None
                    ):
                        with patch.object(
                            theme_manager, "_detect_theme_by_storage", return_value=None
                        ):
                            with patch.object(
                                theme_manager, "_detect_theme_by_meta_tags", return_value=None
                            ):
    ...
    async def test_detect_current_theme_by_style(self, theme_manager, mock_page):
        ...
        with patch.object(theme_manager, "_detect_theme_by_css_classes", return_value=None):
            with patch.object(
                theme_manager, "_detect_theme_by_computed_styles", return_value="dark"
            ):
                with patch.object(theme_manager, "_detect_theme_by_cookies", return_value=None):
                    with patch.object(
                        theme_manager, "_detect_theme_by_url_params", return_value=None
    ...
    async def test_detect_current_theme_exception(self, theme_manager, mock_page):
        ...
        # 模拟检测方法抛出异常
        with patch.object(
            theme_manager, "_detect_theme_by_css_classes", side_effect=Exception("CSS error")
        ):
            result = await theme_manager.detect_current_theme(mock_page)
    ...
    async def test_set_theme_by_cookie_success(self, theme_manager, mock_page):
        ...
        with patch.object(
            theme_manager, "detect_current_theme", side_effect=["light", "light", "dark"]
        ):
    ...
    async def test_set_theme_exception(self, theme_manager, mock_page):
        """测试设置主题时发生异常"""
        with patch.object(
            theme_manager, "detect_current_theme", side_effect=Exception("Detection failed")
        ):
            result = await theme_manager.set_theme(mock_page, "dark")
    ...
    async def test_ensure_theme_before_search_needs_change(self, theme_manager, mock_page):
        ...
        with patch.object(
            theme_manager, "detect_current_theme", new_callable=AsyncMock, return_value="light"
        ):
            with patch.object(
                theme_manager, "set_theme", new_callable=AsyncMock, return_value=True
            ) as mock_set:
                result = await theme_manager.ensure_theme_before_search(mock_page)
    ...
    async def test_detect_theme_by_css_classes_light(self, theme_manager, mock_page):
        ...
        mock_page.query_selector.side_effect = (
            ...
            + [Mock()]
            + [None] * (light_selectors_count - 1)  # 第一个浅色主题选择器找到
        )
    ...
    async def test_detect_theme_by_cookies_other_theme_cookie(self, theme_manager, mock_page):
        """测试其他主题Cookie检测"""
        mock_page.context.cookies.return_value = [{"name": "theme", "value": "dark"}]
    ...
    async def test_detect_theme_by_cookies_none(self, theme_manager, mock_page):
        """测试Cookie检测无结果"""
        mock_page.context.cookies.return_value = [{"name": "other_cookie", "value": "some_value"}]
    ...
    def test_vote_for_theme_tie_default_light(self, theme_manager):
        """测试投票机制 - 平票时默认浅色"""
        detection_results = [("css_classes", "dark"), ("computed_styles", "light")]
    ...
    async def test_detect_current_theme_conflicting_methods(self, theme_manager, mock_page):
        ...
        with patch.object(
            theme_manager, "_detect_theme_by_css_classes", return_value="dark"
        ):  # 权重3
            with patch.object(
                theme_manager, "_detect_theme_by_computed_styles", return_value="dark"
            ):  # 权重3
                with patch.object(
                    theme_manager, "_detect_theme_by_cookies", return_value="light"
                ):  # 权重2
                    with patch.object(
                        theme_manager, "_detect_theme_by_url_params", return_value=None
                    ):
                        with patch.object(
                            theme_manager, "_detect_theme_by_storage", return_value="light"
                        ):  # 权重1
                            with patch.object(
                                theme_manager, "_detect_theme_by_meta_tags", return_value="light"
                            ):  # 权重1
                                result = await theme_manager.detect_current_theme(mock_page)
            ...
            with patch.object(theme_manager, "_set_theme_by_javascript", return_value=True):
                with patch.object(theme_manager, "_set_theme_by_force_css", return_value=False):
                    with patch.object(theme_manager, "_set_theme_by_url", return_value=True):
                        with patch.object(
                            theme_manager, "_set_theme_by_cookie", return_value=False
                        ):
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                with patch.object(
                                    theme_manager, "detect_current_theme", return_value="dark"
                                ):
                                    mock_page.url = "https://www.bing.com"
    ...
    async def test_quick_theme_check_failure(self, theme_manager, mock_page):
        ...
        with patch.object(theme_manager, "_detect_theme_by_css_classes", return_value="light"):
            with patch.object(
                theme_manager, "_detect_theme_by_computed_styles", return_value="light"
            ):
                with patch.object(theme_manager, "_detect_theme_by_cookies", return_value="light"):
                    result = await theme_manager._quick_theme_check(mock_page, "dark")
...
class TestBingThemeManagerFailureHandling:
    ...
    async def test_handle_theme_setting_failure(self, theme_manager, mock_page):
        ...
        with patch.object(theme_manager, "_generate_theme_failure_diagnostic") as mock_diagnostic:
            with patch.object(
                theme_manager, "_generate_failure_suggestions", return_value=["建议1", "建议2"]
            ):
                with patch.object(theme_manager, "_save_failure_screenshot", return_value=True):
                    ...
                    await theme_manager._handle_theme_setting_failure(
                        mock_page, "dark", failure_details
                    )
    ...
    async def test_generate_theme_failure_diagnostic_success(self, theme_manager, mock_page):
        ...
        with patch.object(theme_manager, "detect_current_theme", return_value="light"):
            mock_page.evaluate.side_effect = [
                ...
                False,
                True,
            ]  # page_ready, has_error, network_online

            diagnostic = await theme_manager._generate_theme_failure_diagnostic(
                mock_page, "dark", failure_details
            )
    ...
    async def test_set_theme_with_fallback_force_success(self, theme_manager, mock_page):
        ...
        with patch.object(theme_manager, "set_theme", return_value=False):
            with patch.object(
                theme_manager, "force_theme_application", return_value=True
            ) as mock_force:
                result = await theme_manager.set_theme_with_fallback(mock_page, "dark")
    ...
    async def test_set_theme_with_fallback_css_success(self, theme_manager, mock_page):
        ...
        with patch.object(theme_manager, "set_theme", return_value=False):
            with patch.object(theme_manager, "force_theme_application", return_value=False):
                with patch.object(
                    theme_manager, "_apply_css_only_theme", return_value=True
                ) as mock_css:
                    result = await theme_manager.set_theme_with_fallback(mock_page, "dark")
        ...
        with patch.object(theme_manager, "set_theme", return_value=False):
            with patch.object(theme_manager, "force_theme_application", return_value=False):
                with patch.object(theme_manager, "_apply_css_only_theme", return_value=False):
                    with patch.object(
                        theme_manager, "_apply_minimal_theme_markers", return_value=True
                    ) as mock_minimal:
                        result = await theme_manager.set_theme_with_fallback(mock_page, "dark")
    ...
    async def test_set_theme_with_fallback_all_fail(self, theme_manager, mock_page):
        ...
        with patch.object(theme_manager, "set_theme", return_value=False):
            with patch.object(theme_manager, "force_theme_application", return_value=False):
                with patch.object(theme_manager, "_apply_css_only_theme", return_value=False):
                    with patch.object(
                        theme_manager, "_apply_minimal_theme_markers", return_value=False
                    ):
                        result = await theme_manager.set_theme_with_fallback(mock_page, "dark")
    ...
    async def test_ensure_theme_before_search_with_fallback(self, theme_manager, mock_page):
        ...
        with patch.object(
            ...
            with patch.object(
                theme_manager, "set_theme", new_callable=AsyncMock, return_value=False
            ):
                with patch.object(
                    theme_manager,
    ...
    async def test_set_theme_enhanced_failure_handling(self, theme_manager, mock_page):
        ...
        with patch.object(theme_manager, "detect_current_theme", return_value="light"):
            with patch.object(theme_manager, "_set_theme_by_url", return_value=False):
                with patch.object(theme_manager, "_set_theme_by_cookie", return_value=False):
                    with patch.object(
                        theme_manager, "_set_theme_by_localstorage", return_value=False
                    ):
                        with patch.object(
                            theme_manager, "_set_theme_by_javascript", return_value=False
                        ):
                            with patch.object(
                                theme_manager, "_set_theme_by_settings", return_value=False
                            ):
                                with patch.object(
                                    theme_manager, "_set_theme_by_force_css", return_value=False
                                ):
                                    with patch.object(
                                        theme_manager, "_handle_theme_setting_failure"
                                    ) as mock_handle:
                                        result = await theme_manager.set_theme(mock_page, "dark")
...
class TestBingThemeManagerVerification:
    ...
    async def test_verify_theme_setting_success(self, theme_manager, mock_page):
        ...
        with patch.object(theme_manager, "detect_current_theme", return_value="dark"):
            with patch.object(theme_manager, "_verify_theme_by_all_methods") as mock_verify_methods:
                with patch.object(
                    theme_manager, "_verify_theme_persistence_detailed"
                ) as mock_persistence:
                    # 设置模拟返回值
            ...
            with patch.object(
                ...
                with patch.object(theme_manager, "_detect_theme_by_cookies", return_value="dark"):
                    with patch.object(
                        ...
                        with patch.object(
                            theme_manager, "_detect_theme_by_storage", return_value="dark"
                        ):
                            with patch.object(
                                theme_manager, "_detect_theme_by_meta_tags", return_value=None
                            ):
                                result = await theme_manager._verify_theme_by_all_methods(
                                    mock_page, "dark"
                                )
    ...
    async def test_verify_theme_by_all_methods_with_errors(self, theme_manager, mock_page):
        ...
        with patch.object(theme_manager, "_detect_theme_by_css_classes", return_value="dark"):
            with patch.object(
                theme_manager, "_detect_theme_by_computed_styles", side_effect=Exception("样式错误")
            ):
                with patch.object(theme_manager, "_detect_theme_by_cookies", return_value="light"):
                    with patch.object(
                        theme_manager, "_detect_theme_by_url_params", return_value=None
                        ...
                        with patch.object(
                            theme_manager,
                            "_detect_theme_by_storage",
                            side_effect=Exception("存储错误"),
                        ):
                            with patch.object(
                                theme_manager, "_detect_theme_by_meta_tags", return_value="dark"
                            ):
                                result = await theme_manager._verify_theme_by_all_methods(
                                    mock_page, "dark"
                                )
    ...
    @pytest.mark.asyncio
    async def test_verify_theme_persistence_detailed_refresh_failure(
    ...
    async def test_verify_and_fix_theme_setting_initial_success(self, theme_manager, mock_page):
        """测试验证和修复主题设置 - 初始验证成功"""
        mock_verification = {"success": True, "verification_score": 0.9, "detected_theme": "dark"}
    ...
    @pytest.mark.asyncio
    async def test_verify_and_fix_theme_setting_fix_success_first_attempt(
        self, theme_manager, mock_page
    ):
        """测试验证和修复主题设置 - 第一次修复成功"""
    ...
    @pytest.mark.asyncio
    async def test_verify_and_fix_theme_setting_fix_success_second_attempt(
        self, theme_manager, mock_page
    ):
        """测试验证和修复主题设置 - 第二次修复成功"""
        initial_verification = {"success": False, "verification_score": 0.3}
        ...
        with patch.object(
            theme_manager,
            "verify_theme_setting",
            side_effect=[initial_verification, failed_verification, success_verification],
        ):
            with patch.object(theme_manager, "set_theme", return_value=False):  # 第一次修复失败
                with patch.object(
                    theme_manager, "set_theme_with_retry", return_value=True
                ):  # 第二次修复成功
                    with patch.object(
                        theme_manager, "set_theme_with_fallback", return_value=True
                    ):  # 第三次修复成功
                        result = await theme_manager.verify_and_fix_theme_setting(
                            mock_page, "dark", max_attempts=3
                        )
    ...
    @pytest.mark.asyncio
    async def test_verify_and_fix_theme_setting_fix_success_fallback(
        self, theme_manager, mock_page
    ):
        """测试验证和修复主题设置 - 降级策略成功"""
        initial_verification = {"success": False, "verification_score": 0.3}
        ...
        assert result["fix_attempts"][2]["success"] is True
        assert (
            result["fix_attempts"][2]["verification_after_fix"]["success"] is True
        )  # 最终验证成功

    @pytest.mark.asyncio
    async def test_verify_and_fix_theme_setting_fix_success_second_attempt_with_verification(
        self, theme_manager, mock_page
    ):
    ...
    async def test_verify_and_fix_theme_setting_fix_exception(self, theme_manager, mock_page):
        ...
        with patch.object(
            theme_manager,
            "verify_theme_setting",
            side_effect=[initial_verification, final_verification],
        ):
            with patch.object(theme_manager, "set_theme", side_effect=Exception("设置异常")):
                result = await theme_manager.verify_and_fix_theme_setting(
                    mock_page, "dark", max_attempts=1
                )
