...
class TestBingThemePersistence:
    ...
    async def test_save_theme_state(self, theme_manager, temp_theme_file):
        """测试保存主题状态"""
        theme = "dark"
        context_info = {"user_agent": "test-agent", "viewport": {"width": 1280, "height": 720}}
        ...
        with open(temp_theme_file, encoding="utf-8") as f:
    ...
    def test_validate_theme_state_valid(self, theme_manager):
        """测试验证有效的主题状态数据"""
        valid_data = {"theme": "dark", "timestamp": time.time(), "version": "1.0"}
    ...
    def test_validate_theme_state_invalid_theme(self, theme_manager):
        """测试验证无效主题值的数据"""
        invalid_data = {"theme": "invalid_theme", "timestamp": time.time(), "version": "1.0"}
    ...
    @pytest.mark.asyncio
    async def test_restore_theme_from_state_success(
        self, theme_manager, mock_page, temp_theme_file
    ):
        """测试成功从状态恢复主题"""
        test_data = {
            "theme": "dark",
        ...
        with (
            patch.object(theme_manager, "detect_current_theme", side_effect=detect_calls),
    ...
    @pytest.mark.asyncio
    async def test_restore_theme_from_state_already_correct(
        self, theme_manager, mock_page, temp_theme_file
    ):
        """测试当前主题已经正确时的恢复"""
        test_data = {
    ...
    async def test_ensure_theme_persistence(self, theme_manager, mock_page, mock_context):
        """测试确保主题持久化"""
        with (
            patch.object(theme_manager, "detect_current_theme", return_value="dark"),
    ...
    @pytest.mark.asyncio
    async def test_check_theme_persistence_integrity(
        self, theme_manager, mock_page, temp_theme_file
    ):
        """测试检查主题持久化完整性"""
        test_data = {
    ...
    @pytest.mark.asyncio
    async def test_ensure_theme_before_search_with_persistence(
        self, theme_manager, mock_page, mock_context
    ):
        """测试搜索前确保主题设置（包含持久化）"""
        with (
            patch.object(theme_manager, "restore_theme_from_state", return_value=True),
    ...
    @pytest.mark.asyncio
    async def test_ensure_theme_before_search_restore_failed(
        self, theme_manager, mock_page, mock_context
    ):
        """测试恢复失败时的搜索前主题确保"""
        with (
            patch.object(theme_manager, "restore_theme_from_state", return_value=False),
    ...
    async def test_theme_persistence_workflow(self):
        ...
        try:
            ...
            with (
                ...
                persistence_result = await theme_manager.ensure_theme_persistence(
