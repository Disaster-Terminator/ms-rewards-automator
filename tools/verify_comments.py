...
# 从 GitHub API 获取数据
threads_api = client.fetch_pr_threads("Disaster-Terminator", "RewardsCore", 9)

# 从数据库获取数据
db_path = Path(__file__).parent.parent / ".trae" / "data" / "review_threads.json"
data_db = json.loads(db_path.read_text())
threads_db_dict = data_db.get("threads", {})
threads_db = [threads_db_dict[k] for k in threads_db_dict if k.isdigit()]
