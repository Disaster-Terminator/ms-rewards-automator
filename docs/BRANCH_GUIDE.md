...
├── feature/web-frontend      # Web 前端界面（React + TypeScript）
├── fix/xxx                   # Bug修复分支
...
| `feature/web-frontend` | Web 前端界面 | `frontend/`, `web_server.py`, `src/api/` |
...
| 问题 | 状态 | 说明 |
|------|------|------|
| 停止任务按钮无效 | 🔴 待修复 | `task_service.py` 中任务跟踪逻辑问题 |
| 积分获取失败 | 🟡 依赖 main | `main` 分支积分获取模块选择器问题 |
| 进度显示 0/0 | ✅ 已修复 | 使用 `||` 替代 `??` 运算符 |
| "警告"字样 | ✅ 已修复 | 改为"提示"更委婉 |
